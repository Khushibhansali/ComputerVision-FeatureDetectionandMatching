...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            trans_vec = np.array((-f.pt[0], -f.pt[1], 1))
            transMx = transformations.get_trans_mx(trans_vec)
            rotMx = transformations.get_rot_mx(0, 0, -np.deg2rad(f.angle))
            scaleMx = transformations.get_scale_mx(0.2, 0.2, 1)
            trans_matrix_B = transformations.get_trans_mx(np.array([4, 4, 0]))
            fullMx = np.dot(scaleMx, np.dot(rotMx, transMx))
            
            resMx = np.dot(trans_matrix_B, fullMx)
            transMx = resMx[:2, (0, 1, 3)]
            # TODO-BLOCK-END
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i, desc1Row in enumerate(desc1):
            bestDistance = float('inf')
            for j, desc2Row in enumerate(desc2):
                diffRow = desc1Row - desc2Row
                distance = np.sum(pow(diffRow,2))
                if bestDistance > distance:
                    bestDistance = distance
                    bestMatch = cv2.DMatch(i, j, bestDistance)
